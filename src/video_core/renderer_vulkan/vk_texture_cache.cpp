...
View CachedSurface::CreateViewInner(const ViewParams& params, bool is_proxy) {
    ...
    views[params] = view;
    if (!is_proxy) {
        // view->DecorateViewName(gpu_addr, params.TargetName() + "V:" +
...
void VKTextureCache::ImageCopy(Surface& src_surface, Surface& dst_surface,
    ...
    UNIMPLEMENTED_IF(src_surface->GetSurfaceParams().target == SurfaceTarget::Texture3D);
    UNIMPLEMENTED_IF(dst_surface->GetSurfaceParams().target == SurfaceTarget::Texture3D);
    ...
    src_surface->Transition(copy_params.source_z, copy_params.depth, copy_params.source_level, 1,
                            ...
                            vk::ImageLayout::eTransferSrcOptimal);
    dst_surface->Transition(copy_params.dest_z, copy_params.depth, copy_params.dest_level, 1,
                            vk::PipelineStageFlagBits::eTransfer,
                            vk::AccessFlagBits::eTransferWrite,
                            vk::ImageLayout::eTransferDstOptimal);

    const auto& dld{device.GetDispatchLoader()};
    const vk::ImageSubresourceLayers src_subresource(src_surface->GetAspectMask(),
                                                     copy_params.source_level, copy_params.source_z,
                                                     copy_params.depth);
    const vk::ImageSubresourceLayers dst_subresource(dst_surface->GetAspectMask(),
                                                     copy_params.dest_level, copy_params.dest_z,
                                                     copy_params.depth);
    const vk::Offset3D src_offset(copy_params.source_x, copy_params.source_y, 0);
    const vk::Offset3D dst_offset(copy_params.dest_x, copy_params.dest_y, 0);
    const vk::Extent3D extent(copy_params.width, copy_params.height, 1);
    const vk::ImageCopy copy(src_subresource, src_offset, dst_subresource, dst_offset, extent);
    scheduler.Record([src_image = src_surface->GetImageHandle(),
                      dst_image = dst_surface->GetImageHandle(), copy](auto cmdbuf, auto& dld) {
