...
constexpr FixedPipelineState::InputAssembly GetInputAssemblyState(const Maxwell& regs) {
    constexpr bool primitive_restart_enable = false; // TODO
    return FixedPipelineState::InputAssembly(
        regs.draw.topology, primitive_restart_enable,
...
bool FixedPipelineState::Rasterizer::operator==(const Rasterizer& rhs) const {
    return std::tie(cull_enable, depth_bias_enable, cull_face, front_face) ==
           std::tie(rhs.cull_enable, rhs.depth_bias_enable, rhs.cull_face, rhs.front_face);
}
